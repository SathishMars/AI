...
// PRIMARY INDEX + UNIQUE CONSTRAINT: One conversation per template per account/organization
// Ensures only one conversation history exists for each template context
...
// CONVERSATION ID LOOKUP: Fast retrieval by conversation ID
// Supports queries: Load conversation by deterministic ID
db.aimeWorkflowConversations.createIndex(
  { id: 1 },
  { 
...
// MESSAGE TIMESTAMP: Sort messages chronologically
// Supports queries: Get latest messages in conversation
db.aimeWorkflowConversations.createIndex(
  { account: 1, workflowTemplateID: 1, "message.timestamp": -1 },
  { 
