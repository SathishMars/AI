...
export const getListOfMrfTemplates = async ( input: ListOfMrfTemplatesInput): Promise<string> => {
    ...
    const templates: string[] = (results || []).map((t) => (`- id: ${String(t.id)},  name: ${String(t.name)}, organization: ${t.organization ?? null}`));
...
export const GetListOfMRFTemplatesTool = tool(listToolFunc, {
    ...
    schema: getListOfMrfTemplatesSchema
});