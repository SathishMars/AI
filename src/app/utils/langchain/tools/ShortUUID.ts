...
const shortUUIDSchema = z.object({
    length: z.number().min(8).optional().describe('The length of each UUID. Minimum size is 8'),
    count: z.number().min(1).optional().describe('Number of UUIDs to generate. When >1 returns a comma-separated list'),
});
