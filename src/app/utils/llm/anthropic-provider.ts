// src/app/utils/llm/anthropic-provider.ts
...
export class AnthropicProvider implements LLMProvider {
  ...
  async checkRateLimit(taskType: AITaskType): Promise<boolean> {
    ...
    // Simple rate limiting: max 50 requests per minute per task type (Anthropic has lower limits)
  ...
  private handleError(error: unknown, taskType: AITaskType): Error {
    if (error instanceof Error) {
      console.error(`Anthropic ${taskType} error:`, error.message);
      return new Error(`Anthropic ${taskType} failed: ${error.message}`);
    }
    
    console.error(`Unknown Anthropic ${taskType} error:`, error);
    return new Error(`Anthropic ${taskType} failed with unknown error`);
  }
}