...
export async function GET(request: NextRequest) {
  try {
    // Get account and organization from headers (injected by middleware)
    ...
    const url = new URL(`${railsBaseUrl}/api/v1/requests`);
    ...
    if (!railsResponse.ok) {
      ...
      console.error('[request-templates] Rails API error:', {
        ...
        errorBody: errorText,
        headers: Object.fromEntries(railsResponse.headers.entries()),
      });
      return NextResponse.json(
        { error: 'Failed to fetch templates from Rails', details: errorText },
        { status: railsResponse.status }
      );
    }
