...
const OOS_KEYWORDS: Array<{ type: string; words: string[] }> = [
  ...
  { type: "general_knowledge", words: ["what is", "tell me about", "explain", "define", "cricket", "world cup", "capital of", "weather", "news", "time now", "joke", "poem", "coding", "how to write", "theory", "trivia", "predict", "forecast"] },  // Removed "who is" to allow attendee queries
...
const IN_SCOPE_HINTS: Record<InsightsInScopeCategory, string[]> = {
  ...
  profiles_roles: ["vip", "speaker", "sponsor", "staff", "role", "company", "title", "companies", "vips", "sponsors", "who are the vips", "who are the sponsors", "who are vips and sponsors", "who is registered", "who is that person", "who are registered", "who are the registered", "details of person registered", "person registered"],
...
export function detectScopeAndCategory(question: string): {
  ...
  if (explicitInScopePatterns.some(pattern => pattern.test(q))) {
    ...
    // Registration/Attendee queries about registered people
    if (/who\s+is.*(?:registered|person|people|attendee|that person|those people)/i.test(q) ||
        /who\s+are.*(?:registered|people|attendees)/i.test(q) ||
        /details?\s+of\s+.*(?:person|people|attendee).*registered/i.test(q) ||
        /(?:person|people|attendee).*registered/i.test(q)) {
      return { scope: "in_scope", category: "profiles_roles" };
    }
    return { scope: "in_scope", category: "statistics_summaries" };
  }
  ...
  // More specific patterns that only match clearly OOS queries
  const oosRegexes = [
    /who (?:won|painted|discovered|created|wrote|cleared)/i,  // Block "who won", "who painted", etc. but NOT "who is"
    /who is (?!.*(?:registered|attendee|person|people|that person|those people|vip|sponsor|speaker|next attendee|recently updated|details)).*(?:mahatma|gandhi|president|prime minister|actor|author|scientist|inventor|discovered|created|wrote|painted|won|the|capital|weather)/i,  // Block "who is" only for clearly external queries
  ...
  if (qaOosPhrases.some(p => q.includes(p)) || oosRegexes.some(r => r.test(q))) {
    ...
    const isActuallyInScope =
      ...
      // Registration/Attendee queries - CRITICAL: Allow "who is" queries about registered attendees
      (q.includes("who is") && (q.includes("registered") || q.includes("person") || q.includes("people") || q.includes("attendee") || q.includes("that person") || q.includes("those people") || q.includes("details"))) ||
      (q.includes("who are") && (q.includes("registered") || q.includes("people") || q.includes("attendees"))) ||
      // Details queries about registered attendees
      (q.includes("details") && (q.includes("registered") || q.includes("person") || q.includes("people") || q.includes("attendee"))) ||
      // Travel queries
